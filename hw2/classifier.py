...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        return torch.where(y_proba[:, self.positive_class] >= self.threshold, 1, 0).int()
...
def plot_decision_boundary_2d(
    ...
    x_grid = torch.column_stack((x1, x2))
    y_grid = classifier.classify(x_grid)
    y_hat = y_grid.view(x1_grid.shape)
